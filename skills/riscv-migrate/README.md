...
> x86/ARM → RISC-V（含 RVV）一体化迁移技能。按**四大阶段**推进（扫描 → 分流迁移 → 工程编译 → 性能分析），以**性能最大化**为默认目标（尽可能使用 RVV intrinsic / RISC-V 汇编），性能分析作为独立 Subagent 在前三阶段通过后对全部 asm/RVV/AutoVec 条目默认执行。每个迁移点的状态以 `scan_result.json` 条目里的 `status` / `marking` / `perf` 结构化字段维护，权威源在 JSON，不在源码注释。
...
4. **阶段四（独立 Subagent）**：前三阶段全部通过后召唤性能分析 Subagent，对热点用 `llvm-mca -mcpu=zhufeng2` 分析并迭代优化，优化通过后更新对应条目的 `marking`。
...
- **性能分析**（独立 Subagent）：说「分析热点，目标芯片是朱峰2号」→ agent 用 `llvm-mca -mcpu=zhufeng2` 分析汇编，给出优化建议，并通过更新 `marking` 字段同步结果

agent 会自主管理环境：工具链、QEMU、llvm-mca 均由 `resources/` 下的脚本按需部署，幂等。**首次部署需内网可达**（从内网 Artifactory 下载制品）；制品缓存到本地后复用，不再重复下载；离线场景可预先放置制品包（各脚本均支持跳过下载直接解压）。
...
阶段三: 工程级交叉编译 ─────► RV 可执行程序（QEMU 中可运行）
            │
阶段四(独立 Subagent): 性能分析 ► llvm-mca 报告 + 优化提交（更新 marking）
```

- 阶段二每个条目内部：分流 → 迁移 → 知识库 → 验证 → 快速 mca 门禁
- 阶段二所有条目 status=DONE 后才能进入阶段三
- 阶段三通过后对全部 asm/RVV/AutoVec 条目默认召唤阶段四性能分析 agent（用户叫停才跳过）

## 迁移点状态字段规范

每个条目在 `scan_result.json` 中通过以下结构化字段维护进度：

| 字段 | 取值 | 说明 |
| ---- | ---- | ---- |
| `status` | `TODO` / `START` / `DONE` | 三态互斥 |
| `marking` | string | 备注文本：异常说明 / 性能结论 / TODO(后续) 等；`status=DONE` 时必填。**禁止跨架构 QEMU wall-clock 结论**（口径见 `referens/perf_measure.md`） |
| `perf` | object / null | 结构化性能数据（L1/L2 指标、A/B 变体与胜者）；asm/RVV/AutoVec 条目 `DONE` 时必填，其余条目为 `null` |

```jsonc
// 阶段一扫描产出（默认）：
{ "status": "TODO", "marking": "", "perf": null }

// 阶段二 2.2 进入条目：
{ "status": "START", "marking": "", "perf": null }

// 阶段二 2.4 + 2.4.5 门禁通过（schema 权威见 referens/perf_measure.md 第 4 节）：
{
  "status": "DONE",
  "marking": "无异常；相对同目标标量基线：指令数 -68%，Block RThroughput -41%（zhufeng2）",
  "perf": { "baseline_kind": "scalar_c",
            "metrics": { "instret_per_elem_baseline": 4.2, "instret_per_elem_variant": 1.34,
                         "block_rthroughput_baseline": 9.0, "block_rthroughput_variant": 5.3,
                         "ipc_variant": 4.8, "mcpu": "zhufeng2" },
            "ab_variants": [ { "name": "intrinsic_lmul_m2", "winner": true },
                             { "name": "asm_handwritten", "winner": false,
                               "rejected_reason": "L2 instret 高 3%，可读性差" } ] } }
}
...
```
riscv-migrate/
├── SKILL.md                    # 技能定义（四大阶段 + 性能分析独立 Subagent）
├── referens/
│   ├── project_scan.md         # scan_result.json Schema（含 status/marking/perf/tier 字段与 solver_type 枚举映射）
│   ├── code_migrate.md         # 迁移三步法、指令映射速查表、编译/llvm-mca 详述
